...
/**
 * Created by Programmer on 2017/11/8.
 */

public class CustomSector extends View{
    ...
    public CustomSector(Context context) {
        super(context);
    }

    public CustomSector(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        mPaint = new Paint();
    ...
    public CustomSector(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawLineAndText(Canvas canvas) {
        ...
        for (int i = 0; i < (maxNum < datas.length ? maxNum : datas.length); i++) {
            ...
            //画线条和文字
            drawLine(canvas, start, angles, texts[i], Color.BLACK);
            start += angles;
        }
        //画其他部分的线条和文字
